...
**「暮らしとNotionミートアップ」公式テンプレート**に準拠したデザインルールを採用しています。
...
**暮らしとNotionミートアップ公式テンプレート準拠**のMac風ウィンドウフレームデザインを採用した統一的なLTスライド。

### 共通フレーム要素（全スライド共通）

1. **茶色/ゴールドのフレーム背景**: スライド全体の背景色（`#C4A98D`）
2. **白いウィンドウ**: 角丸の白い矩形 + 黒い太枠線（`#000000`）+ ドロップシャドウ
3. **ダークタイトルバー**: 黒い上部バー（`#000000`）
4. **トラフィックライト**: 赤（`#E0776C`）・黄（`#E5E616`）・緑（`#93D9B4`）の3つのドット（左上）
5. **閉じるボタン**: ×（右上、ヘッダー付きスライドのみ）
6. **グレーヘッダー**: ライトグレー背景（`#F2F2F2`）のスライドタイトル表示エリア
7. **区切り線**: ヘッダーとコンテンツの間のグレー線（`#A5A5A5`）
8. **Lightning Talk バッジ**: 黄色バッジ + 黒枠 + イタリック体（表紙・クロージングスライド）
9. **コピーライト**: `Copyright @ 2024 All Rights Reserved. AAAAAAA Inc.`（下部中央）
...
典型的な構成例:
1. **表紙**（1枚）- タイトル（左側）、アイコン（右側）、スピーカー名
2. **自己紹介**（1枚）- プロフィール画像＋箇条書き（・で区切り）
3. **導入**（1〜2枚）- 今日話すこと、背景
...
- **フォント**: Arial
- **レイアウト**: 16:9
- **フレーム背景**: 茶色/ゴールド（`#C4A98D`）
- **ウィンドウ**: 白い角丸矩形 + 黒い太枠線 + ドロップシャドウ
- **タイトルバー**: 黒（`#000000`）+ トラフィックライト
- **ヘッダー**: ライトグレー（`#F2F2F2`）+ 区切り線
- **バッジ**: 黄色背景 + 黒枠 + イタリック体「Lightning Talk」（表紙・クロージング）
- **箇条書き**: ・（中黒）を使用（自己紹介スライド）
- **表紙レイアウト**: タイトル左側、アイコン右側
- **コピーライト**: `Copyright @ 2024 All Rights Reserved. AAAAAAA Inc.`
